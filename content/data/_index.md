...
`atlas-regional-{96..119}.json` (×24) + `atlas-regional-timeseries.json`: Regional cosponsorship profiles spanning the 96th–119th Congresses (1979–2026), covering the entire Sagebrush-Rebellion era forward. For each of three state-delegation regions (Great Plains, Intermountain & Pacific West + AK, Corn Belt Midwest), counts cosponsorships across six CRS policy areas (Public Lands and Natural Resources, Agriculture and Food, Environmental Protection, Water Resources Development, Energy, Animals), broken out by chamber and party. Two data paths dispatched on Congress number: the modern 108th–119th uses the [GPO BILLSTATUS bulk ZIPs](https://www.govinfo.gov/bulkdata/BILLSTATUS) (keyless, ~500MB cached); the legacy 96th–107th uses the [api.congress.gov](https://api.congress.gov/) v3 JSON API (requires `CONGRESS_API_KEY`, ~186k cached responses). All bill responses cached locally under `scripts/.cache/atlas/` for resumability. The combined `atlas-regional-timeseries.json` (~120KB) is a long-form file with one row per (Congress × region × policy area), with chamber and party breakdowns inline. Feeds the [Regional Trends](/atlas/regional-trends/) atlas entry. The 96th Congress is the institutional floor — House cosponsorship was rule-prohibited until 1967 and cosponsor-count-capped until 1979.
